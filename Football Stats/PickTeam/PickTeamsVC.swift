...
//              btndropdown.isHidden = true
   ...
   let flowLayout = UICollectionViewFlowLayout()
           flowLayout.itemSize = CGSize(width: UIScreen.main.bounds.width/2-130, height:160)
       ...
       let bgImage = UIImageView();
        ...
        btnrandom.layer.borderColor =  UIColor.black.cgColor
        btnrandom.layer.borderWidth = 1;
        btnindexselection.clipsToBounds = true
       btnindexselection.layer.cornerRadius = 6;
       btnindexselection.layer.borderColor =   UIColor.init(red: 2.0/255.0, green: 148.0/255.0, blue: 44.0/255.0, alpha: 1).cgColor
       btnindexselection.layer.borderWidth = 1;
        btndraft.clipsToBounds = true
      btndraft.layer.cornerRadius = 6;
      btndraft.layer.borderColor =   UIColor.init(red: 2.0/255.0, green: 148.0/255.0, blue: 44.0/255.0, alpha: 1).cgColor
      btndraft.layer.borderWidth = 1;
     btndefaultdatabase.clipsToBounds = true
     btndefaultdatabase.layer.cornerRadius = 8;
        btndefaultdatabase.layer.borderColor =  UIColor.black.cgColor
    ...
    func PickteamsCall()
         {
            ...
            if NetworkState.isConnected()
            {
                ...
                let sessionid =  UserDefaults.standard.object(forKey: "Sessionid")
                  ...
                  let verify_param = ["sessionID":sessionid as Any,"storedProcedureName":"getPickTeams","input1":str2 as Any ,"input2":btndefaultdatabase.currentTitle!,"input3":strInt as Any] as [String : Any]
                       let signin_headers: HTTPHeaders = ["x-api-key":"CODEX@123"]
                 AF.request(GlobalConstants.ApiURL, method: .post, parameters: verify_param, encoding: URLEncoding.httpBody, headers:signin_headers).responseJSON {
                             response in
                            DispatchQueue.main.async{
                ...
                if NetworkState.isConnected()
                {
                    hud = MBProgressHUD.showAdded(to: self.view, animated: true)
                              ...
                              let sessionid =  UserDefaults.standard.object(forKey: "Sessionid")
                                ...
                                let verify_param = ["sessionID":sessionid as Any,"storedProcedureName":"sp_ConfirmDraftSelection","input1":strInt1 as Any ,"input2":storedversionnumber.text!] as [String : Any]
                                    let signin_headers: HTTPHeaders = ["x-api-key":"CODEX@123"]
                              AF.request(GlobalConstants.ApiURL, method: .post, parameters: verify_param, encoding: URLEncoding.httpBody, headers:signin_headers).responseJSON {
                                          response in
                                         DispatchQueue.main.async{
                 ...
                 let sessionid =  UserDefaults.standard.object(forKey: "Sessionid")
                   ...
                   let strInt = Int(storedTeams_id)
                    //let str2 =  UserDefaults.standard.object(forKey: "registerid")
                           // let string = btndateofmatich.currentTitle
                        //String(UserDefaults.standard.integer(forKey: "registerid"))
                    let verify_param = ["sessionID":sessionid as Any,"storedProcedureName":"proc_helpSelection","input1":strInt as Any ,"input2":btnrandom.currentTitle!] as [String : Any]
                        let signin_headers: HTTPHeaders = ["x-api-key":"CODEX@123"]
                  AF.request(GlobalConstants.ApiURL, method: .post, parameters: verify_param, encoding: URLEncoding.httpBody, headers:signin_headers).responseJSON {
                              response in
                             DispatchQueue.main.async{
